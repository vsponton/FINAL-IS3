...
function App() {
  ...
  // =================== RENDER HELPERS ===================
  // Extraídos para evitar ternarios anidados (Sonar: S3358).

  const renderBooksContent = () => {
    if (loadingBooks) {
      return <div style={{ padding: "0.75rem" }}>Cargando...</div>;
    }
    if (books.length === 0) {
  ...
  const renderLoansContent = () => {
    if (loadingLoans) {
      return <div style={{ padding: "0.75rem" }}>Cargando préstamos...</div>;
    }
    if (loans.length === 0) {
  ...
  const renderSalesContent = () => {
    if (loadingSales) {
      return <div style={{ padding: "0.75rem" }}>Cargando ventas...</div>;
    }
    if (sales.length === 0) {
